...
- **langsmith** - трейсинг LangChain и evaluation в LangSmith (через переменные `LANGSMITH_*`)
- **ragas** (>=0.2.0), **datasets** - batch-оценка качества RAG и загрузка feedback в LangSmith
...
│   ├── dataset_synthesizer.py  # Синтез Q&A датасета из PDF/JSON
│   └── evaluation.py   # RAGAS evaluation и feedback в LangSmith
├── data/               # PDF для индексации; в ДЗ модуля 4 — также JSON (см. idea.md)
├── datasets/           # Синтезированные датасеты (например 05-rag-qa-dataset.json)
...
2. **handlers.py** - обработка событий
   ...
   - `/evaluate_dataset` - оценка RAG на датасете LangSmith (RAGAS)
   - Обработчик всех текстовых сообщений → вызов RAG → сохранение ответа в историю
   - При `SHOW_SOURCES=true` — блок «📚 Источники» с файлами и страницами
...
4. **rag.py** - RAG-логика
   ...
   - `get_rag_chain()` / `rag_answer(messages)` - цепочка возвращает `answer` и `documents`
   - `format_sources(documents)` - компактный список источников для Telegram
   - Промпты для conversation и query transformation
   - Использует retriever из vector_store

5. **config.py** - конфигурация
   - Класс Config: LLM/эмбеддинги, `SHOW_SOURCES`, `LANGSMITH_*`, `RAGAS_LLM_MODEL`, `RAGAS_EMBEDDING_MODEL`
...
**Мониторинг:** при `LANGSMITH_TRACING_V2=true` LangChain автоматически отправляет traces в LangSmith; отдельный код трейсинга не нужен.
