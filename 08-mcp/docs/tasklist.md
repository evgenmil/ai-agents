...
| 5 | 19 | MCP-сервер mcp-bank-agent (search_products, loan_calc) | 🚧 В работе | 11.06.2026 |
| 5 | 20 | Подключение MCP-инструментов к агенту | 🚧 В работе | 11.06.2026 |
| 5 | 21 | Системный промпт и разграничение инструментов | 🚧 В работе | 11.06.2026 |
| 5 | 22 | Финальная полировка и документация MCP | 🚧 В работе | 11.06.2026 |
...
## 🚀 Спринт 5: MCP-интеграция банковского агента

Референс: `slides/notebook/09-mcp/agent-mcp.ipynb` (раздел «Агент с MCP инструментами»).  
Цель: вынести динамические данные о продуктах и калькулятор кредита в MCP-сервер; подключить к агенту через `MultiServerMCPClient`.

### Итерация 19: MCP-сервер mcp-bank-agent

**Цель:** Подпроект `mcp/mcp-bank-agent` с инструментами `search_products` и `loan_calc`

- [x] Создать `mcp/mcp-bank-agent/pyproject.toml` (fastmcp, uv)
- [x] Создать `mcp/mcp-bank-agent/server.py` — FastMCP, streamable-http, порт 8000
- [x] Инструмент `search_products(query, product_type)` — поиск по `data/bank_products.json`
- [x] Сгенерировать `data/bank_products.json` (вклады, кредиты, карты, счета; данные с sberbank.com)
- [x] Инструмент `loan_calc(loan_amount, interest_rate, term_months)` — API Ninjas Mortgage Calculator
- [x] Добавить `make run-mcp-bank` в корневой Makefile
- [x] `make install` синхронизирует зависимости MCP-подпроекта

**Тест:** `make run-mcp-bank` → сервер на `http://localhost:8000/mcp`; tools/list содержит `search_products` и `loan_calc`

---

### Итерация 20: Подключение MCP-инструментов к агенту

**Цель:** `MultiServerMCPClient` + graceful degradation; async инициализация

- [x] Добавить `langchain-mcp-adapters>=0.1.0` в корневой `pyproject.toml`
- [x] `async def create_bank_agent()` и `async def initialize_agent()`
- [x] `mcp_tools = await mcp_client.get_tools()` с `transport: streamable_http`
- [x] `tools = [rag_search, currency_convert] + mcp_tools`
- [x] Graceful degradation: при ошибке MCP — warning в лог, работа без MCP-инструментов
- [x] `MCP_BANK_URL` в `config.py` (по умолчанию `http://localhost:8000/mcp`)
- [x] `bot.py`: `await agent.initialize_agent()`

**Тест:** MCP запущен → агент видит 4 инструмента; MCP остановлен → агент стартует с 2 локальными инструментами

---

### Итерация 21: Системный промпт и разграничение инструментов

**Цель:** Агент знает, когда вызывать `rag_search`, `search_products`, `loan_calc`, `currency_convert`

- [x] Обновить `prompts/agent_system.txt`: правила выбора инструмента
- [x] Few-shot: ставки по вкладам → `search_products`; досрочное погашение → `rag_search`; расчёт платежа → `loan_calc`
- [x] Указать параметры и примеры вызова MCP-инструментов

**Тест:** «Какие ставки по вкладам?» → `search_products`; «Досрочное погашение?» → `rag_search`; «Платёж 500 000 на 3 года под 19.9%?» → `loan_calc`

---

### Итерация 22: Финальная полировка и документация MCP

**Цель:** Актуальная документация, README, проверка двухтерминального запуска

- [x] Обновить `docs/idea.md`, `docs/vision.md`, `.cursor/rules/conventions.mdc`
- [ ] Обновить `README.md`: двухтерминальный запуск, MCP-архитектура, `API_NINJAS_KEY`
- [ ] Проверить `make install`, `make run-mcp-bank`, `make run`
- [ ] Прогон диалогов: продукты + расчёт кредита + graceful degradation

**Тест:** полный прогон с MCP; бот без MCP; зафиксировать наблюдения
