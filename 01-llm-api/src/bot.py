...
class ChatBot:
    ...
    def _add_usage(self, usage: Optional[dict]):
        if not usage:
            return
    ...
    def summarize_history(self):
        ...
        if len(self.conversation_history) - start <= KEEP_RECENT:
            return

        prefix = self.conversation_history[:start]
        to_summarize = self.conversation_history[start:-KEEP_RECENT]
        recent = self.conversation_history[-KEEP_RECENT:]

        if not to_summarize:
            return

        transcript = self._format_dialogue(to_summarize)
        console.print("[yellow]Сжимаю историю диалога...[/yellow]")

        try:
            response = self.client.chat.completions.create(
                model=self.model_name,
                messages=[
                    {"role": "system", "content": "Кратко резюмируй эту переписку в 2-3 предложениях."},
                    {"role": "user", "content": transcript},
                ],
            )
            summary = response.choices[0].message.content
            self._add_usage(response.usage.model_dump() if response.usage else None)

            console.print(Panel(
                summary,
                title="📝 Резюме диалога",
                border_style="yellow",
                padding=(1, 2),
            ))
            console.print()

            self.conversation_history = prefix + [
                {"role": "user", "content": f"Краткое резюме предыдущего диалога:\n{summary}"}
            ] + recent
    ...
    def add_message(self, role: str, content: str):
        ...
        start = self._history_start()
        if len(self.conversation_history) - start > MAX_MESSAGES:
