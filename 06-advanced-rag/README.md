...
- 🧠 **Advanced RAG** - три режима retrieval: semantic, hybrid (BM25 + semantic), hybrid+reranker (Cross-Encoder)
...
- `EMBEDDING_MODEL` - модель для создания эмбеддингов (при `EMBEDDING_PROVIDER=openai`)
- `RETRIEVAL_MODE` - режим поиска: `semantic`, `hybrid`, `hybrid+reranker`
- `EMBEDDING_PROVIDER` - `openai` или `huggingface`
- `CROSS_ENCODER_MODEL` - модель reranker (для `hybrid+reranker`)
...
│   └── rag.py          # RAG-логика: LCEL-цепочки, query transformation
├── prompts/
...
2. **Обработка вопроса**:
   ...
   → Retrieval (semantic / hybrid / hybrid+reranker) → Генерация ответа с контекстом
   ```
